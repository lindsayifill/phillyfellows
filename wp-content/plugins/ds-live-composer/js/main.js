 ...
 */
		...
		dslcEditableAreas.each(function(){
			...
			if ( jQuery(this).data('type') == 'simple' ) {
				...
				dslcPenOpts = {
					...
					class: 'dslc-pen',
					list: [
						'blockquote', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'insertorderedlist', 'insertunorderedlist',
 	...
 	function dslc_editable_content_gen_code( dslcField ) {
		...
		dslcContent = dslcField.html().trim();		
 ...
 */
	...
	function dslc_scroller_prev( scroller ) {
		...
		var scrollerNew = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')').prev('.dslca-scroller-item:visible').index();
	...
	function dslc_scroller_next( scroller ) {
		...
		var scrollerCurr = scroller.data('current');
		var scrollerNew = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')').next('.dslca-scroller-item:visible').index();
 ...
 */
	...
	function dslc_module_dragdrop_init() {
		...
		jQuery( '.dslca-modules .dslca-module' ).draggable({
			...
			start : function( event, ui ) {
				if ( bowser.name == 'Firefox' || ( bowser.name == 'Chrome' && parseInt( bowser.version ) >= 32 ) ) {
		...
		jQuery( '.dslc-content' ).sortable({
			...
			start: function(e, ui){
				...
				ui.placeholder.html('<span class="dslca-placeholder-help-text"><span class="dslca-placeholder-help-text-inner">MODULES ROW</span></span>');
		...
		jQuery( '.dslc-modules-section-inner' ).sortable({
			...
			start: function(e, ui){
				...
				// Placeholder
				ui.placeholder.html('<span class="dslca-placeholder-help-text"><span class="dslca-placeholder-help-text-inner">MODULES AREA</span></span>');
	...
	function dslc_filter_module_options( dslcOptionsType ) {
		...
		if ( dslcOptionsType == 'responsive' ) {
			...
			dslc_show_module_options_tab( 'tablet_responsive' );
	...
	function dslc_show_module_options_tab( dslcTabID ) {
		...
		// Tablet
		if ( dslcTabID == 'tablet_responsive' ) {
			jQuery('body').addClass('dslc-res-tablet');
		}

		// Phone
		if ( dslcTabID == 'phone_responsive' ) {
			jQuery('body').addClass('dslc-res-phone');
		}

		// If responsive reload module
		if ( dslcTabID == 'tablet_responsive' || dslcTabID == 'phone_responsive' ) {
 ...
 */
	...
	function dslc_edit_modules_section_colorpicker() {
		...
		/*
		jQuery('.dslca-modules-section-edit-field-colorpicker').ColorPicker({
			...
			onChange: function( hsb, hex, rgb ){
				...
				var dslcField, dslcFieldID, dslcEl, dslcModulesSection, dslcVal, dslcRule, dslcSetting, dslcTargetEl;

				dslcField = jQuery( jQuery(this).data('colorpicker').el );
				dslcField.val( '#' + hex );
				dslcFieldID = dslcField.data('id');
	...
	function dslc_copy_modules_section( modulesSection ) {
		...
		modulesSection.clone().appendTo('.dslc-content').find('.dslc-module-front').css({
			'-webkit-animation-name' : 'none',
			'-moz-animation-name' : 'none',
			'animation-name' : 'none',
			'animation-duration' : '0',
...
jQuery(document).ready(function($) {
	 ...
	 */
		...
		$(document).on( 'click', '.dslca-delete-modules-section-hook', function(e){
			...
			if ( ! $(this).hasClass('dslca-action-disabled') ) {
				dslc_delete_modules_section( jQuery(this).closest('.dslc-modules-section') );
			}
		...
		$(document).on( 'click', '.dslca-edit-modules-section-hook', function(e){
			...
			if ( ! $(this).hasClass('dslca-action-disabled') ) {
				...
				if ( jQuery('.dslca-module-being-edited.dslca-module-change-made').length ) {
					...
					dslc_js_confirm( 'edit_in_progress', '<span class="dslca-prompt-modal-title">You are currently editing a module</span><span class="dslca-prompt-modal-descr">You need to either <strong>confirm</strong> or <strong>cancel</strong> those changes before continuing.</span>', jQuery(this) );

				} else if ( jQuery('.dslca-modules-section-being-edited.dslca-modules-section-change-made').length ) {

					dslc_js_confirm( 'edit_in_progress', '<span class="dslca-prompt-modal-title">You are currently editing a modules row</span><span class="dslca-prompt-modal-descr">You need to either <strong>confirm</strong> or <strong>cancel</strong> those changes before continuing.</span>', jQuery(this) );	
		...
		$(document).on( 'change', '.dslca-modules-section-edit-field', function() {
			...
			var dslcField, dslcFieldID, dslcEl, dslcModulesSection, dslcVal, dslcValReal, dslcRule, dslcSetting, dslcTargetEl;
	...
	$(document).on( 'click', '.dslca-close-composer-hook', function(e){
		...
		if ( ! $('body').hasClass('dslca-saving-in-progress') ) {
			dslc_js_confirm( 'disable_lc', '<span class="dslca-prompt-modal-title">You are about to exit Live Composer</span><span class="dslca-prompt-modal-descr">Any unsaved changed will be lost.</span>', $(this).attr('href') );
		}
	...
	$(document).on( 'click', '.dslca-section-title', function(e){
		...
		if ( $('.dslca-section-title-filter', this).length ) {
			...
			$('.dslca-section-title-filter-options:visible').css({
				'-webkit-animation-name' : 'dslcBounceOut',
				'-moz-animation-name' : 'dslcBounceOut',
				'animation-name' : 'dslcBounceOut',
				'animation-duration' : '0.7s',
			...
			$('.dslca-section-title-filter-options:not(:visible)').show().css({
				opacity : 1,
				'-webkit-animation-name' : 'dslcBounceIn',
				'-moz-animation-name' : 'dslcBounceIn',
				'animation-name' : 'dslcBounceIn',
	...
	$(document).on( 'click', '.dslca-module-edit-hook', function(e){
		...
		if ( ! $('body').hasClass( 'dslca-composer-hidden' ) ) {
			...
			if ( $('.dslca-module-being-edited.dslca-module-change-made').length ) {
				...
				dslc_js_confirm( 'edit_in_progress', '<span class="dslca-prompt-modal-title">You are currently editing another module</span><span class="dslca-prompt-modal-descr">You need to either <strong>confirm</strong> or <strong>cancel</strong> those changes before continuing.</span>', $(this) );

			} else if ( $('.dslca-modules-section-being-edited.dslca-modules-section-change-made').length ) {

				dslc_js_confirm( 'edit_in_progress', '<span class="dslca-prompt-modal-title">You are currently editing a modules row</span><span class="dslca-prompt-modal-descr">You need to either <strong>confirm</strong> or <strong>cancel</strong> those changes before continuing.</span>', $(this) );
	...
	$(document).on('focus', '.dslca-module-edit-field-colorpicker, .dslca-modules-section-edit-field-colorpicker', function(){
	...
	$(document).on( 'mouseenter', '.dslc-modules-section', function(e){
		if ( ! $('body').hasClass('dslca-drag-in-progress') && ! $('body').hasClass('dslca-composer-hidden' ) ) {
			$(this).siblings('.dslc-modules-section:not(.dslca-modules-area-being-deleted)').stop().animate({
				opacity : 0.5
			}, 500);
		}
	}).on( 'mouseleave', '.dslc-modules-section', function(e){
	...
	$(document).on( 'mouseenter', '.dslc-modules-area', function(e){
		if ( ! $('body').hasClass('dslca-drag-in-progress') && ! $('body').hasClass('dslca-composer-hidden' ) ) {
			$(this).siblings('.dslc-modules-area:not(.dslca-modules-area-being-deleted)').stop().animate({
				opacity : 0.5
			}, 500);
		}
	}).on( 'mouseleave', '.dslc-modules-area', function(e){
	...
	$(document).on( 'mouseenter', '.dslc-module-front', function(e){
		
		if ( ! $('body').hasClass('dslca-drag-in-progress') && ! $('body').hasClass('dslca-composer-hidden' ) ) {

			$(this).siblings('.dslc-module-front:not(.dslca-module-being-deleted)').stop().animate({
				opacity : 0.5
			}, 500);
		}

	}).on( 'mouseleave', '.dslc-module-front', function(e){
	...
	$(document).on( 'keydown', function(e){
		if ( ( e.which || e.keyCode ) == 116 ) {
			if ( jQuery('.dslca-save-composer-hook').is(':visible') || jQuery('.dslca-module-edit-save').is(':visible') ) {
				e.preventDefault();
				dslc_js_confirm( 'disable_lc', '<span class="dslca-prompt-modal-title">You are about to refresh the page</span><span class="dslca-prompt-modal-descr">Any unsaved changed will be lost.</span>', document.URL );
			}
		}
	});
