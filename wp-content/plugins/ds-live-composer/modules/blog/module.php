...
class DSLC_Blog extends DSLC_Module {
	...
	var $module_id = 'DSLC_Blog';
	var $module_title = 'Blog';
	...
	function output( $options ) {
		 ...
		 */
			...
			// Posts container
			$container_class = 'dslc-posts dslc-blog-posts dslc-clearfix dslc-blog-posts-type-' . $options['type'] .' dslc-blog-posts-orientation-' . $options['orientation'] . ' ';
		 ...
		 */
			...
			if ( $dslc_query->have_posts() ) {
					...
					while ( $dslc_query->have_posts() ) : $dslc_query->the_post(); $count += $increment; $real_count++;
						...
						<div class="<?php echo $element_class . $columns_class . $extra_class; ?>" data-cats="<?php echo $post_cats_data; ?>">
							...
							<?php if ( $post_elements == 'all' || in_array( 'thumbnail', $post_elements ) ) : ?>
								...
								<div class="dslc-blog-post-thumb">
				...
				if ( $dslc_is_admin ) :
					?><div class="dslc-notification dslc-red">You do not have any blog posts at the moment. Go to <strong>WP Admin &rarr; Posts</strong> to add some. <span class="dslca-refresh-module-hook dslc-icon dslc-icon-refresh"></span></span></div><?php
				endif;
